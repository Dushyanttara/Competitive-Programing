...
def prefix_sums(A):
    ...
    Args:
        A (List): List of base elements

    Returns:
        List: List of prefix sums for each index
    """    
    n = len(A)
    P = [0] * (n + 1)
...
def count_total(P,x,y):
    ...
    Args:
        P (list):  List of prefix sums
        x (int): slice start index
        y (int): slice end index

    Returns:
        list: List of difference of slices
    """    
    return P[y + 1] - P[x]
...
sample_array = [2,3,7,5,1,3,9]
start_pos = 4
total_moves = 6

answer = mushrooms(sample_array, start_pos, total_moves)
